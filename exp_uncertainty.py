...
# # fit the data
# uncertainty_model_delta = ComputationalModels(uncertainty_reward_means, uncertainty_reward_sd,
#                                                  model_type='decay_fre', condition='Gains')
# uncertainty_results_delta = uncertainty_model_delta.fit(uncertainty_dict, num_iterations=1000)
#
#
# # unpack the results
# result = pd.DataFrame(uncertainty_results_delta)
...
# best_bounds = []
#
# potential_beta_upper = [x for x in np.arange(0, 5.1, 0.5) if x != 0]
# potential_beta_lower = [x for x in np.arange(-5, 0, 0.5) if x != 0]
...
#
#     aic = np.mean([result['AIC'] for result in uncertainty_results_decayfre])
#     bic = np.mean([result['BIC'] for result in uncertainty_results_decayfre])
#
#     best_bounds.append([beta_lower, beta_upper, aic, bic])
#
# best_bounds = pd.DataFrame(best_bounds, columns=['beta_lower', 'beta_upper', 'AIC', 'BIC'])
#
