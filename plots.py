...
# Supplementary Figure S1a: Order Effects
data['order'] = data['order'].replace({'BF': 'Baseline-Frequency', 'FB': 'Frequency-Baseline'})
data_S1a = data.groupby(['Subnum', 'order', 'Condition', 'TrialType'])['BestOption'].mean().reset_index()
data_S1a['TrialType'] = pd.Categorical(data_S1a['TrialType'], categories=['AB', 'CD', 'CA', 'CB', 'AD', 'BD'], ordered=True)

palette_S1a = {"Baseline-Frequency": sns.color_palette()[2], "Frequency-Baseline": sns.color_palette(palette)[3]}
g = sns.catplot(data=data_S1a, x="TrialType", y="BestOption", hue="order", col="Condition", kind="bar", dodge=True,
                errorbar=('ci', 95), n_boot=10000, height=4, aspect=1, palette=palette_S1a, legend_out=True)
g.set_axis_labels("Trial Type", "% of Optimal Choices")
g.set_titles("{col_name}", size=20)
for ax in g.axes.flat:
    ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.2f'))  # 2 decimals
    ax.tick_params(axis="both", labelsize=15)
    # add horizontal line at 0.5
    ax.axhline(0.5, color=sns.color_palette(palette)[7], linestyle='--', label='Random Chance')
# add legend for horizontal line into the overall legend
handles, labels = ax.get_legend_handles_labels()
if 'Random Chance' not in labels:
    handles.append(plt.Line2D([0], [0], color=sns.color_palette(palette)[7], linestyle='--'))
    labels.append('Random Chance')
g._legend.remove()
g.fig.legend(handles, labels, loc='center right', fontsize=15, title='Order', title_fontsize=18)
g.set_xlabels(size=18)
g.set_ylabels(size=18)
g.fig.subplots_adjust(left=0.10, right=0.70)
plt.savefig('./figures/Figure_S1a.png', dpi=600)
plt.show()

# Supplementary Figure S1b: Training Performance per Condition and Block
data_S1b = data.groupby(['Subnum', 'order', 'Condition', 'Phase', 'TrialType'])['BestOption'].mean().reset_index()
data_S1b1 = data_S1b[data_S1b['TrialType'].isin(['AB', 'CD'])].copy()
data_S1b2 = data_S1b[data_S1b['TrialType'].isin(['CA', 'CB'])].copy()
data_S1b3 = data_S1b[data_S1b['TrialType'].isin(['AD', 'BD'])].copy()

g = sns.catplot(
    data=data_S1b3,
    x="Phase", y="BestOption",
    hue="order",
    row="TrialType", col="Condition",
    kind="point",
    errorbar=('ci', 95), n_boot=10000,
    markers=["o", "s"], linestyles=["-", "--"],
    dodge=False,  # not needed with lines
    palette=palette_S1a,
...
g.set_xlabels('')
g.set_ylabels('')
g.fig.supxlabel("Block", fontsize=20)
# g.fig.supylabel("% of Optimal Choices", fontsize=20)
